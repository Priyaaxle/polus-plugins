...
def feature_extraction(label_image, features, seg_file_names1, embeddedpixelsize, img_emb_uint, unitLength, pixelsPerunit, pixelDistance=5, intensity_image=None):
    ...
    def all(seg_img, units, int_img):
        ...
        if seg_img.ndim == 3:
            logger.warning('Perimeter, orientation, eccentricity, feret diameter, neighbors, hexaonality_score, hexaonality_sd, polygonality_score is not implemented for 3d images.')
        if seg_img.ndim == 2:
            #calculate perimeter
    ...
    if label_image.ndim ==3:
        if (features == ['perimeter'] and len(features) ==1):
            raise ValueError('Perimeter is not implemented for 3d images.')
        if (features == ['orientation'] and len(features) ==1):
            raise ValueError('Orientation is not implemented for 3d images.')
        if (features == ['eccentricity'] and len(features) ==1):
            raise ValueError('Eccentricity is not implemented for 3d images.')
            
        
    for each_feature in features:
       if label_image.ndim ==3:
           if each_feature == 'perimeter':
               logger.warning('Perimeter is not implemented for 3d images.')
               continue
           if each_feature == 'orientation':
               logger.warning('Orientation is not implemented for 3d images.')
               continue
           if each_feature == 'eccentricity':
               logger.warning('Eccentricity is not implemented for 3d images.')
               continue
       #Lists all the labels in the image
       label1 = [r.label for r in regions]
       label = [lb for lb in label1]
       
       #List of labels for only objects that are not touching the border
       label_nt = [nt_border.label for nt_border in regions1]
       label_nt_touching = [label_value for label_value in label_nt]
           ...
           if embeddedpixelsize:
               ...
               if label_image.ndim == 2:
                   df.columns =[
                    ...
                    df.columns =[
                       'Volume_%s^3'%unitLength,
                       'Centroid row',
                       'Centroid column',
                       'Convex area_%s^3'%unitLength,
                       'Equivalent diameter_%s'%unitLength,
                       'Euler number',
                       'Kurtosis',
                       'Major axis length_%s'%unitLength,
           ...
           if 'Area' or 'Convex area' or 'Equivalent diameter' or 'Major axis length' or 'Maxferet' or 'Minor axis length' or 'Minferet' or 'Perimeter' in df.columns:
               if embeddedpixelsize:
                   ...
                   if label_image.ndim == 2:
                       ...
                       df.rename({
                           "area": "Volume_%s^3"%unitLength,
                           "convex_area": "Convex area_%s^3"%unitLength,
                           "equivalent_diameter": "Equivalent diameter_%s"%unitLength,
                           "major_axis_length": "Major axis length_%s"%unitLength,
                           "minor_axis_length": "Minor axis length_%s"%unitLength,
                            }, axis='columns', inplace=True)
